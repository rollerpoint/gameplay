(function(window, document, $, undefined) {
  ...
  $.urlParam = function (name) {
    ...
    $(function() {
      ...
      var $userInfo = $('.user-info-wrap');
      var $resetAlert = $userInfo.find('.user-info-wrap');
      $userInfo.find('.reset-password').on('click',function() {
        var email = $userInfo.data('email');
        $userInfo.find('#wpcrl_rp_email').val(email);
        $userInfo.find('button[type="submit"]').click();
      });
      $userInfo.find('#wpcrlResetPasswordForm').on('success.form.fv', function(e) {
          // start loading
      }).on('success_send',function(e,data) {
        setTimeout(function() {
          location.reload()
        },1500)
      }).on('error_send',function(e,data) {
        console.log(e,data);
      });

      $('.open-vacancy-btn').click(function() {
          $('.vacancy-short').addClass('hidden');
          $('.vacancy-full').removeClass('hidden');
      });
      $('.close-vacancy-btn').click(function() {
          $('.vacancy-short').removeClass('hidden');
          $('.vacancy-full').addClass('hidden');
      });
      ...
      $memberModal.find('.modal-footer .btn').on('click',function() {
        $memberModal.find('form').find('.submit').click();
      });
      
      var saveCompanyAttachment = function(attachment_id,$choser) {
        var $form = $choser.closest('form');
        var name = $choser.find('input').attr('name');
        $choser.addClass('whirl loader traditional');
        $.ajax('/wp-admin/admin-ajax.php',{
          type:'POST',
          data:{
            action:"save_company_attachment",
            name:name,
            attachment_id:attachment_id
          }
        }).done(function(result) {
          $choser.removeClass('whirl loader traditional');
        }).error(function(){
          $choser.removeClass('whirl loader traditional');
          console.warn(err);
        }); 
      }
      $('.my-company-blog-form').on('beforeAjaxSerialize',function() {
        var $self = $(this);
        var $form = $(this);
        var $choser = $self.find('input[name="attachment_id"]').closest('.wp-chose-image');
        var attachment = $choser.data('attachment');
        if (attachment) {
          var url = attachment.url;
          if (attachment.sizes['ruvod-publication-body']) {
            url = attachment.sizes['ruvod-publication-body'].url;
          }
          var $new_img = $('<img>').css({
            'max-width':420,
          }).attr('src', url).addClass('alignleft');
          var $content = $("<div>");
          if (tinymce.get('post_content')) {
            $(tinymce.get('post_content').getContent()).appendTo($content);
          } else {
            $content.html($form.find('#post_content').text());
          }
          if ($content.find('p:first-child').length) {
            $content.find('p:first-child').first().prepend($new_img);
          } else {
            $content.prepend($new_img);
          }
          if (tinymce.get('post_content') && !tinymce.get('post_content').hidden) {
            tinymce.get('post_content').setContent($content.html());
          } else {
            $form.find('#post_content').val($content.html());
            $form.find('#post_content').trigger('change');
          }
        }
      });
      var pasteThumbnailInContent = function(attachment,$choser) {
        var $form = $choser.closest('form');
        var name = $choser.find('input').attr('name');
        ...
        var $img = $content.find('img:first-child').first();
        if ($img.length) {
          $img.replaceWith($new_img);
        } else {
          // сохраним на потом
          return $choser.data('attachment',attachment);
          $content.prepend($new_img);
        }
        if (tinymce.get('post_content') && !tinymce.get('post_content').hidden) {
          tinymce.get('post_content').setContent($content.html());
      ...
      $('.wp-chose-image').each(function() {
        ...
        $self.on('click',function() {
          if ($alert.data('attachment')) {
            $alert.hide();
            $alert.data('attachment',false);
          }
          var dlg = wp.media({
          ...
          dlg.on('close',function() {
            ...
            if (attachment) {
              if (min) {
                var width = min.split('x')[0];
                var height = min.split('x')[1];
                if (attachment.width < width || attachment.height < height) {
                  $alert.text('Минимальный размер ' + min);
                  $alert.show();
                  $alert.data('attachment',true);
                  $('html').scrollTop($alert.offset().top - 100);
                  return false;
                }
              }
              ...
              if ($self.closest('.company-blog-form').length) {
                saveCompanyAttachment(attachment.id,$self);
              }
              if ($self.closest('.company-blog-post-form').length) {
                pasteThumbnailInContent(attachment,$self);
              }
            }
        ...
        $forms.each(function(i,form) {
          ...
          $form.ajaxForm({
            beforeSerialize: function($form, options) { 
              $form.trigger('beforeAjaxSerialize')
              try {
                tinyMCE.triggerSave();
              } catch(err) {
