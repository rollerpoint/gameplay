    ...
    if ($vacancy) {
      ...
      if ($data['payment_published']) {
        ...
        if ($paid_at_str) {
          ...
          $now =  new DateTime();
          if ($paid_end <= $now) {
            $data['payment_published'] = null;
          }
        }
      }
      if ($vacancy->post_status == 'draft' && $data['payment_published']) {
        $payment_draft = true;
        $data['payment_published'] = null;
      }
    } 
  ...
  <form enctype="multipart/form-data" class="ajax-form has-nested"  action="/wp-admin/admin-ajax.php" method="post">
    ...
    <div class="row">
      <div class="col-sm-6 col-xs-12">
        <div class="form-group">
            <label for="title">
              <?php _e('Position', RUVOD_TEXT_DOMAIN) ?>
      ...
      <div class="col-sm-12 col-xs-12">
        <div class="form-group">
            ...
            <input required type="text" data-placeholder="<?php _e('Add competencies', RUVOD_TEXT_DOMAIN) ?>" data-source="/wp-admin/admin-ajax.php?action=skills_list" class="tagsinput form-control" name="skills" value="<?php echo $data['skills'] ?>" id="skills">
        </div>
      </div>
      <div class="col-sm-12 col-xs-12">
        <div class="form-group">
          <label for="demands">
            <?php _e('Requirements', RUVOD_TEXT_DOMAIN) ?>
            <span class="reqiured-sym">
              *
            </span>
          </label>
      ...
      <div class="col-sm-12 col-xs-12">
        <div class="form-group">
          <label for="duties">
            <?php _e('Official duties', RUVOD_TEXT_DOMAIN) ?>
            <span class="reqiured-sym">
              *
            </span>
          </label>
      ...
      <div class="col-sm-12 col-xs-12">
        <div class="form-group">
          <label for="conditions">
            <?php _e('Working conditions', RUVOD_TEXT_DOMAIN) ?>
            <span class="reqiured-sym">
              *
            </span>
          </label>
      ...
      <div class="col-sm-6 col-xs-12">
        <div class="form-group">
            <label for="contacts_email">
              <?php _e('Email', RUVOD_TEXT_DOMAIN) ?>
              <span class="reqiured-sym">
                *
              </span>
            </label>
    ...
    <div class="row">
      <div class="col-sm-12 form-footer">
        ...
        <?php if ($data['payment_published'] == '1') { ?>
            <input type="hidden" value="<?php echo $data['payment_published'] ?>" name="payment_published">
            <button type="submit" name="to_draft" value="1" class="btn btn-secondary submit">
                <?php _e('Hide', RUVOD_TEXT_DOMAIN) ?>
            </button>
        <?php } else if(!ruvod_company_vacancy_is_posible_payment_publish() && !$payment_draft) { ?>
            <button type="submit" name="payment_published" value="1" disabled class="btn btn-danger submit">
                <?php _e('Publish', RUVOD_TEXT_DOMAIN) ?>
            </button>
            <?php $msg=__('Not enough RVD on balance',RUVOD_TEXT_DOMAIN) ?>
            <?php $msg.= '<br>'.__('To post additional vacancies, fill in the account',RUVOD_TEXT_DOMAIN) ?>
            <a class="text-red" href="#" data-html="true" onclick="return false;" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="<?php echo $msg ?>">(?)</a>
        <?php    } else {?>
            <button type="submit" name="payment_published" value="1" class="btn btn-danger submit">
                <?php _e('Publish', RUVOD_TEXT_DOMAIN) ?>
            </button>
        <?php } ?>
