...
add_action('wp_ajax_add_or_create_user_member', 'ruvod_ajax_add_or_create_user_member');

function ruvod_ajax_add_or_create_user_member() {
    $email = $_POST['email'];
    $target_user = get_user_by( 'email', $email );
    if ($target_user) {
        ruvod_ajax_add_user_member();
    } else {
        ruvod_ajax_create_user_member();
    }
}
...
function ruvod_ajax_create_user_member() {
    ...
    if (!is_wp_error($new_user_id)) {
        ...
        $message.= '<br>';
        $message.= '<strong>' . __('Username : ',RUVOD_TEXT_DOMAIN) . '</strong>' . $email;
        $message.= '<br>';
