...
<div class="form-wrapper my-company-blog-form">
  <form enctype="multipart/form-data" class="ajax-form company-blog-post-form"  action="/wp-admin/admin-ajax.php" method="post">
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-4 form-file-wrap">
            ...
            <div class="form-group">
                <div class="wp-chose-image" data-size="post-thumbnail">
      ...
      <div class="col-sm-8 col-xs-8">
        <div class="form-group pull-up">
            <label>
                <?php _e('Post title', RUVOD_TEXT_DOMAIN) ?>
            ...
            <select data-no-results-text="<?php _e('No results match', RUVOD_TEXT_DOMAIN) ?>" name="post_categories[]" id="post_categories[]" class="chosen form-control" multiple data-placeholder=" ">
                <?php 
                    $catq = array(
                        'meta_query'  => array(
                            array(         
                                'key' => 'hide_for_company',  
                                'compare' => 'NOT EXISTS'
                            )
                        )
                
                    );
                    $cats = get_categories( $catq ); 
    ...
    <div class="row">
      ...
      <div class="col-sm-12 col-xs-12 form-footer">
        ...
        <?php } ?>
        <?php if (!$data['payment_published'] && !ruvod_company_post_is_posible_payment_publish()) { ?>
            <div class="inline">
                <div class="checkbox">
                    <input type="checkbox" disabled>
                    <label>
                        <?php echo __('To main', RUVOD_TEXT_DOMAIN) ?>
                    </label>
                </div>
                <input type="hidden" value="1" name="payment_published" <?php echo ($data['payment_published'] == '1' ? '' : 'disabled') ?>>
            </div>
            <?php $msg=__('You can not post news on the main page',RUVOD_TEXT_DOMAIN) ?>
            <?php $msg.= '<br>'.__('In a week you can send 1 publication',RUVOD_TEXT_DOMAIN) ?>
        ...
        <?php } else { ?>
            <div class="inline">
                <div class="checkbox">
                    ...
                    ?>
                    <input value="1" id="payment_published" name="payment_published" type="checkbox" <?php echo join(' ',$attrs) ?>>
                    <label for="payment_published">
                        <?php echo $data['payment_published'] == '1' ? __('On main', RUVOD_TEXT_DOMAIN) : __('To main', RUVOD_TEXT_DOMAIN) ?>
                    </label>
                </div>
                <input type="hidden" value="1" name="payment_published" <?php echo ($data['payment_published'] == '1' && $data['status'] != 'pending' ? '' : 'disabled') ?>>
            </div>
        <?php } ?>
