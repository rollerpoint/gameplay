...
function ruvod_enqueue($hook) {
	...
	wp_enqueue_style('ruvod-local-fonts', plugins_url( '/assets/fonts.css', __FILE__ ) );
...
function register_ruvod_plugin_settings() {
	...
	register_setting( 'ruvod-options-group', 'ruvod_vacancy_channel');
...
function ruvodplugin_options() {
  ...
  <table class="form-table">
	 <tbody>
		...
		<tr>
           <th scope="row"><label for="ruvod_yandex_receiver">Канал вакансий телеграм</label></th>
           <td>
					 		<input style="width: 350px;" type="text" name="ruvod_vacancy_channel" value="'.get_option('ruvod_vacancy_channel').'"/>
					 </td>
		</tr>
		<tr>
...
{	
	...
	if (is_page( 'blogs' )) {
		$page_template = dirname( __FILE__ ) . '/page-blogs.php';
	}
...
add_filter('digezine_posts_list_class','ruvod_add_list_classes');

function ruvod_add_list_classes($classes) {
	global $wp_query;
	if ($wp_query->max_num_pages == 1) {
		$classes[] = 'without-paginate'; 
	}
	return $classes;
}
...
function my_c2c_autohyperlink_urls_filters( $filters ) {
    return array('the_content');
}

add_filter( 'c2c_autohyperlink_urls_filters', 'my_c2c_autohyperlink_urls_filters' );

function ruvod_enqueue_tinymce_plugins_scripts($plugin_array)
{
	//enqueue TinyMCE plugin script with its ID.
	$scripts_path = RUVOD_PLUGIN_DIRNAME . "/assets/tinymce_customs.js";
	$ver = filemtime( $scripts_path );
	
    $plugin_array["incut_button_plugin"] =  plugin_dir_url(__FILE__) . "assets/tinymce_customs.js?v=".$ver;
    return $plugin_array;
}

add_filter("mce_external_plugins", "ruvod_enqueue_tinymce_plugins_scripts");

function ruvod_register_buttons_editor($buttons)
{
    //register buttons with their id.
    array_push($buttons, "incut");
    return $buttons;
}

add_filter("mce_buttons", "ruvod_register_buttons_editor");

function ruvod_register_editors_styles() {
	$styles_path = RUVOD_PLUGIN_DIRNAME . "/assets/tinymce_customs.css";
	$ver = filemtime( $styles_path );
	add_editor_style(plugin_dir_url(__FILE__).'assets/tinymce_customs.css?ver='.$ver );
}
add_action( 'current_screen', 'ruvod_register_editors_styles' );
