...
class Digezine_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'digezine' );
		$this->widget_id          = 'digezine_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'digezine' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'digezine' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'digezine' ),
				'label' => esc_html__( 'Subscribe Title', 'digezine' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'digezine' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'digezine' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'digezine' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'digezine' ),
				'label' => esc_html__( 'Subscribe submit label', 'digezine' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'digezine' ),
				'label' => esc_html__( 'Subscribe success', 'digezine' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'digezine' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'digezine' ),
				'label' => esc_html__( 'Follow Title', 'digezine' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'digezine' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'digezine' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'digezine' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'digezine' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'digezine' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'digezine' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'digezine' ),
					'top-center'    => esc_html__( 'Top Center', 'digezine' ),
					'top-right'     => esc_html__( 'Top Right', 'digezine' ),
					'center-left'   => esc_html__( 'Middle Left', 'digezine' ),
					'center'        => esc_html__( 'Middle Center', 'digezine' ),
					'center-right'  => esc_html__( 'Middle Right', 'digezine' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'digezine' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'digezine' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'digezine' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'digezine' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'digezine' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'digezine' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'digezine' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'digezine' ),
					'contain' => esc_html__( 'Contain', 'digezine' ),
					'auto'    => esc_html__( 'Auto', 'digezine' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'digezine' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_digezine_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_digezine_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'digezine' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'digezine' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'digezine' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'digezine' ),
		);

		$this->nonce = wp_create_nonce( 'digezine-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'digezine_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'digezine_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'digezine' );
		}
	...
	public function get_social_nav() {
		return digezine_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = digezine_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'digezine-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'digezine_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function digezine_register_subscribe_follow_widgets() {
	register_widget( 'Digezine_Subscribe_Follow_Widget' );
}
