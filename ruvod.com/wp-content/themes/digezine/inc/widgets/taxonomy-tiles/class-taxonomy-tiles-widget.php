...
if ( ! class_exists( 'Digezine_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Digezine_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'digezine' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'digezine' );
			$this->widget_id          = 'digezine-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				'title' => array(
					...
					'label' => esc_html__( 'Widget title', 'digezine' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category',
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'digezine' ),
				...
				'post_tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'digezine' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'digezine' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'digezine' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'digezine' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'digezine' ),
						),
					),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'digezine' ),
				...
				'items_padding' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'digezine' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'digezine_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'digezine_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'digezine' ),
				...
				echo apply_filters( 'digezine_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'digezine_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function digezine_register_taxonomy_tiles_widget() {
		register_widget( 'Digezine_Taxonomy_Tiles_Widget' );
	}
}
