 ...
 * @package Digezine
 */
if ( ! class_exists( 'Digezine_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Digezine_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'digezine' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'digezine' );
			...
			$this->settings           = array(
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'digezine' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'digezine' ),
						'title'         => esc_html__( 'Post Title', 'digezine' ),
						'comment_count' => esc_html__( 'Comment Count', 'digezine' ),
					),
					'label'   => esc_html__( 'Post sorted', 'digezine' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'digezine' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'digezine' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'digezine_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'digezine_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'digezine_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'digezine_image_grid_widget_before',
				...
				echo apply_filters( 'digezine_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'digezine_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function digezine_register_image_grid_widget() {
		register_widget( 'Digezine_Image_Grid_Widget' );
	}
}
