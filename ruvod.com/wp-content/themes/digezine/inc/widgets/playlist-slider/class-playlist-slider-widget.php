...
if ( !class_exists( 'Digezine_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Digezine_Playlist_Slider_Widget.
	 */
	class Digezine_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name = esc_html__( 'Playlist Slider', 'digezine' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'digezine' );
			$this->widget_id = apply_filters( 'digezine_playlist_slider_widget_ID', 'digezine_widget_playlist_slider' );
			$this->widget_cssclass = apply_filters( 'digezine_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility = digezine_utility()->utility;
			$this->settings = array(
					'title'              => array(
							'type'  => 'text',
							'value' => esc_html__( 'Playlist Slider', 'digezine' ),
					...
					'width'              => array(
							'type'  => 'text',
							'value' => '100%',
							'label' => esc_html__( 'Slider width ( px, %, rem )', 'digezine' ),
					),
					'height'             => array(
							'type'  => 'text',
							'value' => '500',
							'label' => esc_html__( 'Slider height ( px, rem )', 'digezine' ),
					),
					'thumbnail_controls' => array(
							...
							'value'       => true,
							'options'     => array(
									false => esc_html__( 'Hide', 'digezine' ),
									true  => esc_html__( 'Buttons', 'digezine' ),
							),
							'label'       => esc_html__( 'Select Thumbnail Controls', 'digezine' ),
							'placeholder' => esc_html__( 'Select Thumbnail Controls', 'digezine' ),
					),
					'slider_controls'    => array(
							...
							'label'       => esc_html__( 'Select Slider Controls', 'digezine' ),
							'placeholder' => esc_html__( 'Select Thumbnail Controls', 'digezine' ),
					),
					'title_length'       => array(
							'type'       => 'stepper',
					...
					'thumb_title_length' => array(
							...
							'min_value'  => '0',
							'step_value' => '1',
							'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'digezine' ),
					),
					'mate_data'          => array(
							'type'    => 'checkbox',
							'value'   => array(
							...
							'options' => array(
									...
									'comments' => esc_html__( 'Comment count', 'digezine' ),
					...
					'thumb_mate_data'    => array(
							...
							'label'   => esc_html__( 'Display thumbnail post meta data', 'digezine' ),
					),
			);
			...
			$this->layout_settings = apply_filters( 'digezine_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
						'size'        => 'digezine-thumb-s',
						'mobile_size' => 'digezine-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
						...
						'buttons'            => apply_filters( 'digezine_playlist_buttons', false ),
...
add_action( 'widgets_init', 'digezine_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function digezine_register_playlist_slider_widget() {
	register_widget( 'Digezine_Playlist_Slider_Widget' );
}
