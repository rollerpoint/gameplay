 ...
 * @return string
 */
function digezine_get_main_menu() {
	$args = apply_filters( 'digezine_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'digezine_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'digezine' ),
	) );
 ...
 * @return void
 */
function digezine_main_menu() {
	$main_menu = digezine_get_main_menu();
 ...
 * @return void
 */
function digezine_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'digezine_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="linearicon linearicon-chevron-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'digezine' ) . '</span>',
		'button-close-inner-html' => '<i class="linearicon linearicon-cross"></i>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = digezine_get_main_menu();
 ...
 * @return void
 */
function digezine_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', digezine_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'digezine_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'digezine_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'digezine' ),
	) );
 ...
 * @return void
 */
function digezine_top_menu() {

	if ( ! get_theme_mod( 'top_menu_visibility', digezine_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		return;
	}
	...
	wp_nav_menu( apply_filters( 'digezine_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
 ...
 * @return string
 */
function digezine_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'digezine_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'digezine_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'digezine' ),
	), $context, $type );
 ...
 * @return null|string
 */
function digezine_set_nav_menu( $args ) {
 ...
 * @return void
 */
function digezine_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'digezine_menu_toggle_html',
 ...
 * @return void
 */
function digezine_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'digezine_vertical_menu_toggle_html',
