 ...
 * @package Digezine
 */
if ( ! class_exists( 'Digezine_About_Author_Widget' ) ) {

	/**
	 * Class Digezine_About_Author_Widget.
	 */
	class Digezine_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'digezine widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'digezine' );
			$this->widget_id          = 'digezine_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'digezine' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'digezine' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'digezine' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'digezine' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'digezine' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'digezine' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'digezine' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'digezine' ),
					'label' => esc_html__( 'Link label', 'digezine' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'digezine' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'digezine-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'digezine_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'digezine_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function digezine_register_about_author_widgets() {
	register_widget( 'Digezine_About_Author_Widget' );
}
