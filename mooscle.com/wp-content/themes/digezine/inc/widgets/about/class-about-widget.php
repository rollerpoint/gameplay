...
if ( ! class_exists( 'Digezine_About_Widget' ) ) {

	/**
	 * Class Digezine_About_Widget.
	 */
	class Digezine_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'digezine' );
			$this->widget_id          = 'digezine_widget_about';
			$this->widget_name        = esc_html__( 'About Digezine', 'digezine' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'digezine' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'digezine' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'digezine' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'digezine' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'digezine' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, digezine_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'digezine_widget_about_template_file', 'inc/widgets/about/views/about.php' ), false, false );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'digezine_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'digezine_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'digezine_register_about_widget' );
/**
 * Register about widget.
 */
function digezine_register_about_widget() {
	register_widget( 'Digezine_About_Widget' );
}
