...
class Wptelegram_Post_Handler {
	 ...
	 * @since  	1.0.0
	 * @access 	private
	 * @var WPTelegram_Telegram_Api $tg_api Telegram API Object
	 */
	private $tg_api;
	...
	public function handle_save_post( $post_id, $post ) {
	    ...
	    if ( ! $is_cron ) {
	    	// Return if user has not permissions to edit pages
			if ( 'page' == $post->post_type && ! current_user_can( 'edit_page', $post_id ) ){
				return;
			}
			// Return if user has not permissions to edit posts
	...
	private function get_message_template( $post, $from = 'options' ) {
		...
		if ( $is_template && 'on' == $this->override_switch ) {
			$template = wptelegram_sanitize_message_template( $_POST['wptelegram_message_template'], 'noJSON' );
			$json = false;
		} elseif ( 'postmeta' == $from ) {
	...
	private function prepare_message( $post, $post_id, $template ) {
		...
		$send_featured_image = $this->options['message']['send_featured_image'];
		if ( 'on' == $send_featured_image && has_post_thumbnail( $post_id ) ) {
			// post thumbnail ID
			$thumbnail_id = get_post_thumbnail_id( $post_id );

			// featured image url
			$featured_image_url = wp_get_attachment_url( $thumbnail_id );
			$featured_image_url = apply_filters( 'wptelegram_post_featured_image_url', $featured_image_url, $post );
		...
		$this->prepare_post_data( $text, $parse_mode, $featured_image_url, $caption );
	}
	 ...
	 * @param $featured_image_url string
	 *
	 */
	private function prepare_post_data( $text, $parse_mode, $featured_image_url, $caption ){
		...
		if ( $featured_image_url ) {
			...
			if ( 'before' == $image_position ) {
				...
				if ( 'on' == $attach_image && null != $parse_mode ) {
					$text = $this->add_hidden_image_url( $text, $featured_image_url, $parse_mode );
			...
			if ( isset( $method_params['sendPhoto'] ) ) {
				$method_params['sendPhoto']['photo'] = $featured_image_url;
	...
	private function send( $method_params ){
		...
		$this->tg_api = new WPTelegram_Telegram_Api( $this->options['telegram']['bot_token'] );
		...
		foreach ( $this->chat_ids as $chat_id ) {
			...
			if ( is_wp_error( $res ) ) {
				set_site_transient( 'wptelegram_http_error', $res, 45 );
